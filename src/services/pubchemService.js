 ...
 * @returns {Promise<Object>} Compound data from PubChem
 */
export async function fetchCompoundByCid(cid, imageVersion = null) {
  try {
    // Fetch detailed compound information by CID using property API
    const properties = [
      'MolecularFormula',
    ...
    if (!propertyResponse.ok) {
      throw new Error(`Failed to fetch compound properties for CID ${cid}: ${propertyResponse.statusText}`);
    }
    ...
    const compoundName = propertiesList.IUPACName || `CID ${cid}`;
    ...
    // Generate image URLs
    const version = imageVersion !== null ? imageVersion : PUBCHEM_IMAGE_VERSION;
    const image2DUrl = `https://pubchem.ncbi.nlm.nih.gov/image/imgsrv.fcgi?cid=${cid}&t=l&version=${version}`;
    const image2DUrlSmall = `https://pubchem.ncbi.nlm.nih.gov/image/imgsrv.fcgi?cid=${cid}&t=s&version=${version}`;
    const image3DUrl = `https://pubchem.ncbi.nlm.nih.gov/image/img3d.cgi?&cid=${cid}&t=l`;
    const image3DUrlSmall = `https://pubchem.ncbi.nlm.nih.gov/image/img3d.cgi?&cid=${cid}&t=s`;
    ...
    console.error(`Error fetching compound by CID ${cid}:`, error);
    throw error;
  }
}
